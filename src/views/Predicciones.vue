  ...
  <div class="bg-black vh-100 text-white overflow-hidden">
    <div class="container-fluid px-0 h-100">
      <div class="row g-0 h-100">
        <aside class="d-none d-lg-block col-lg-3 col-xl-2 vh-100 overflow-hidden">
          <Sidebar />
        </aside>

        <main class="col-12 col-lg-9 col-xl-10 vh-100 overflow-hidden px-3 pt-1 pb-1">
          <section class="container-fluid h-100 d-flex flex-column overflow-hidden">
            <div class="sticky-top bg-black z-3 pb-3">
              ...
              <div class="d-flex justify-content-between align-items-center gap-3 mb-4">
                <div class="d-flex align-items-center gap-2 ms-lg-auto flex-nowrap">
                  <select v-model="filterType" class="form-select bg-dark text-white border-success w-auto">
                    <option value="group">Filtrar por grupo</option>
                    <option value="team">Filtrar por selección</option>
                  </select>

                  <select v-if="filterType === 'group'" v-model="selectedGroup" class="form-select bg-dark text-white border-success w-auto">
                    <option value="Todos">Todos los grupos</option>
                    <option v-for="group in groups" :key="group" :value="group">{{ group }}</option>
                  </select>

                  <select v-if="filterType === 'team'" v-model="selectedTeam" class="form-select bg-dark text-white border-success">
              ...
              <div class="card prediction-card text-white rounded-4 mb-4">
                <div class="card-body">
                  <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                    <div>
                      <strong>Primera fase <span class="text-success">· {{ nombreLigaActiva }}</span></strong>
                      <p class="text-white-50 mb-0">
                        <strong>Recuerda:</strong> Solo se contabilizan las predicciones realizadas con al menos 1 hora antes del inicio de cada partido.
                      </p>
                    </div>
                    ...
                    <div class="d-flex align-items-center gap-3">
                      ...
                      <strong>{{ savedMatches.length }}/{{ filteredMatches.length }}</strong>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="flex-grow-1 overflow-auto overflow-x-hidden pb-4 scroll-clean">
              ...
              <div v-else v-for="grupo in partidosAgrupados" :key="grupo.fecha" class="mb-5">
                <h4 class="text-white mb-3 fw-bold border-start border-success border-4 ps-2 ms-1">
                  📅 {{ formatDate(grupo.fecha) }}
                </h4>

                <div class="row g-3">
                  <div v-for="match in grupo.partidos" :key="match.id" class="col-12 col-lg-6">
                    <div class="card prediction-card text-white rounded-4 h-100" :class="{ 'card-locked': esPartidoBloqueado(match) }">
                      <div class="card-body">
                        <div class="text-center mb-3">
                          <small class="text-white-50 fw-semibold">Grupo {{ match.group }}</small>
                        </div>

                        <div class="d-flex align-items-center justify-content-center gap-4">
                          <div class="d-flex align-items-center gap-2">
                            <div class="d-flex align-items-center justify-content-end gap-2">
                              <span class="fw-bold text-end team-name">{{ match.homeTeam }}</span>
                              <img :src="`https://flagcdn.com/w80/${getFlagCode(match.homeTeam)}.png`" :alt="match.homeTeam" class="team-flag" />
                              ...
                              <input
                                ...
                                :class="match.saved ? 'text-success' : 'text-warning'"
                                placeholder="-"
                          ...
                          <div class="fw-bold fs-3" :class="{ 'text-white-50': esPartidoBloqueado(match) }">{{ match.time }}</div>
                          ...
                          <div class="d-flex align-items-center justify-content-start gap-2">
                            <input
                              ...
                              @change="guardarPronostico(match)"
                            />
                            <img :src="`https://flagcdn.com/w80/${getFlagCode(match.awayTeam)}.png`" :alt="match.awayTeam" class="team-flag" />
                            <span class="fw-bold text-start team-name">{{ match.awayTeam }}</span>
                          </div>
                        </div>
                        ...
                        <div class="text-center mt-3">
                          <span 
                            class="badge" 
                            :class="esPartidoBloqueado(match) ? 'bg-danger-subtle text-danger border border-danger' : (match.saved ? 'text-bg-success' : 'text-bg-warning')"
                          >
                            {{ esPartidoBloqueado(match) ? "Bloqueado 🔒" : (match.saved ? "Guardada ✓" : "Pendiente") }}
                          </span>
                        </div>

                        <div class="text-center mt-2">
                          <small class="text-white-50">{{ match.stadium }} · {{ match.city }}</small>
                        </div>
                      </div>
                    </div>
                  </div>
